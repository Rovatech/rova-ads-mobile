...
if (!empty($_GET["restaurant_user_id"]) && !empty($_GET["customer_type"]) && !empty($_GET["customer"]) && !empty($_GET["cart"]) ) {
		...
		$customer_address = urldecode($_GET["customer_address"]);
		$customer_phone = urldecode($_GET["customer_phone"]);
		$order_tax = $_GET["order_tax"];
		$order_amount = $_GET["order_amount"];
		
		
		$result = $orderObj->insertDeliveryOrder($cart, $restaurant_user_id, $customer_type, $customer, $order_table, $order_type, $order_datetime, $customer_instruction, $estimated_time, $order_feedback, $order_status, $customer_name, $customer_address, $customer_phone, $order_tax, $order_amount);
		if($result)
		{
			$response["success"] = 1;
