...
contract('FastFlow', function(accounts) {
  ...
  function printBalance() {
    ...
    .then(function(balance) {
      console.log("Owner balance: ", web3.fromWei(ownerBalance, "ether").toString(), " ETHER / ", balance.valueOf(), " ROVA");
      return instance.balanceOf.call(buyer); 
    }).then(function(balance) {
      console.log("Buyer balance: ", web3.fromWei(buyerBalance, "ether").toString(), " ETHER / ", balance.valueOf(), " ROVA");
      return instance.balanceOf.call(Crowdsale.address); 
    }).then(function(balance) {
      console.log("Crowdsale balance: ", web3.fromWei(crowdsaleBalance, "ether").toString(), " ETHER / ", balance.valueOf(), " ROVA");
    })
  ...
  it("Buy 5,994,000 coins", function() {
    ...
    return Crowdsale.deployed().then(function(crowd) {
       return crowd.sendTransaction({from: buyer, to: crowd.address, value: investSum}).then(function(txn) {
        ...
        var count = parseInt(investSum * (ROVA_PER_ETHER) / (web3.toWei(1, "ether")));
    ...
    return RovaCoin.deployed().then(function(coin) {
      return coin.balanceOf.call(buyer).then(function(oldBalance) {
        return Crowdsale.deployed().then(function(crowd) {
          ...
          var count = parseInt(web3.toWei(100, "finney") * (ROVA_PER_ETHER) / (web3.toWei(1, "ether")));
  ...
  it("Invoke getRemainCoins {from: onwer}", function() {
    ...
    return Crowdsale.deployed().then(function(crowd) {
       ...
       return crowd.getRemainCoins({from: owner}).then(function(txn) {
        ...
        console.log("Crowdsale balance: ", balance.valueOf(), " ROVA");
