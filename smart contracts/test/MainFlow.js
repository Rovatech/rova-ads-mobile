...
var ROVA_PER_ETHER = 6000000000;
var RECEIVE_ROVA_AMOUNT = SEND_ETHER * ROVA_PER_ETHER + ((SEND_ETHER * ROVA_PER_ETHER) / 5); // + 20% bonus
...
contract('MainFlow', function(accounts) {
  ...
  it("should put 1,000,000,000.000000 RovaCoin in the owner account", function() {
    return RovaCoin.deployed().then(function(instance) {
  ...
  it("Send 600,000,000.000000 RovaCoin to Crowdsale contract", function() {
  ...
  it("Buy 100,000,000 coins", function() {
    return Crowdsale.deployed().then(function(crowd) {
        ...
        console.log("Buyer balance: ", balance.valueOf(), " Rova");
        assert.equal(balance.valueOf(), RECEIVE_ROVA_AMOUNT, RECEIVE_ROVA_AMOUNT + " wasn't in the first account");
     });
  });
  ...
  it("Try to reserve the payments {from: buyer}", function() {
    return RovaCoin.deployed().then(function(coin) {
      return coin.balanceOf.call(buyer).then(function(balance) {
        return Crowdsale.deployed().then(function(crowd) {
          console.log('Buyer ROVA: ' + balance.valueOf());
  ...
  it("Try to buy too more coins {from: buyer}", function() {
    return Crowdsale.deployed().then(function(crowd) {
       return crowd.sendTransaction({from: buyer, to: crowd.address, value: web3.toWei(CROWDSALE_CAP/ROVA_PER_ETHER+1, "ether")}).then(function(txn) {
          assert(false, "Throw was supposed to throw but didn't.");
       })
     }).catch(function(error) {
        console.log("Throw was happened. Test succeeded.");
  ...
  it("Buy 6,000 coins without bonus", function() {
    ...
    return Crowdsale.deployed().then(function(crowd) {
       return crowd.sendTransaction({from: buyer, to: crowd.address, value: web3.toWei(1, "ether")}).then(function(txn) {
        ...
        console.log("Buyer balance: ", balance.valueOf(), " ROVA");
        assert.equal(balance.valueOf(), RECEIVE_ROVA_AMOUNT + ROVA_PER_ETHER, RECEIVE_ROVA_AMOUNT + ROVA_PER_ETHER + " wasn't in the first account");
     });
  });
  ...
  it("Try to invoke backRovaCoinOwner {from: buyer}", function() {
    return Crowdsale.deployed().then(function(crowd) {
      return crowd.backRovaCoinOwner({from: buyer}).then(function() {
        assert(false, "Throw was supposed to throw but didn't.");
      }).catch(function(error) {
        console.log("Throw was happened. Test succeeded.");
      });
  ...
  it("Invoke backRovaCoinOwner {from: Crowdsale contract}", function() {
    return Crowdsale.deployed().then(function(crowd) {
      return crowd.backRovaCoinOwner().then(function() {
        return RovaCoin.deployed().then(function(coin) {
          return coin.owner.call().then(function(coinOwner) {
            console.log("RovaCoin owner was changed to: " + coinOwner);
            assert.equal(coinOwner, owner, "RovaCoin owner addresws must be equals to Crowdsale owner address");
          })              
        })
      }).catch(function(error) {
  ...
  it("Invoke backRovaCoinOwner one more time {from: Crowdsale contract}", function() {
    return Crowdsale.deployed().then(function(crowd) {
      return crowd.backRovaCoinOwner().then(function() {
        assert(false, "Throw was supposed to throw but didn't.");
      }).catch(function(error) {
